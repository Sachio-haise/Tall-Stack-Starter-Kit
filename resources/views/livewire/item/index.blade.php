<div>
    ...
    <tr>
        ...
        <th class="py-1 pe-3 text-start" scope="col">
            <div class="flex items-center gap-x-2">
                <span class="flex text-xs font-semibold uppercase tracking-wide text-gray-800 dark:text-gray-200">
                    Name <svg class="-mr-0.5 h-4 w-4 cursor-pointer md:ml-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" wire:click="sortBy('name')">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9"></path>
                    </svg>
                </span>
            </div>
        ...
        @if (!$super_admin && in_array('items_status', $livePermission))
    ...
    <tr class="{{ isset($item->deleted_at) ? 'text-gray-300' : 'text-gray-900' }} border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-600" :key="{{ $item->id }}">
        ...
        <td class="h-px w-px whitespace-nowrap">
            <div class="py-2 pe-6">
                <a class="text-sm dark:text-gray-400" href="#">{{ $item->description }}</a>
            </div>
        </td>
        ...
        <td class="h-px w-px whitespace-nowrap">
            <div class="flex justify-end px-6 py-1.5">
                @if ($item->deleted_at)
                @if (in_array('items_restore', $livePermission) || $super_admin)
                @php
                $restore = App\Http\Helpers\FormHelper::getRestore('items', $item->id);
                @endphp
                {!! $restore !!}
                @else
                ...
                @endif
                @else
                @if (in_array('items_edit', $livePermission) || $super_admin)
                @php
                $edit = App\Http\Helpers\FormHelper::getEdit('items', $item->id);
                @endphp
                {!! $edit !!}
                @else
                <a class="cursor-not-allowed text-gray-500 opacity-30" href="#">Edi</a>
                @endif
                @endif
                &nbsp;
                @if (in_array('items_delete', $livePermission) || $super_admin)
                <form id="deleteForm{{ $item->id }}" style="display: none;" action="{{ route('items.destroy', $item->id) }}" method="post">
                    @csrf
                    @method('DELETE')
                </form>
